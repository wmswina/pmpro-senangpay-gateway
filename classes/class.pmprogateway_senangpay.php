...
if (!function_exists('SenangPay_Pmp_Gateway_load')) {
    ...
    DEFINE('KKD_SENANGPAYPMP', "senangpay-paidmembershipspro");
        ...
        if (!class_exists('PMProGateway_SenangPay')) {
            ...
            class PMProGateway_SenangPay extends PMProGateway
            {
                ...
                static function plugin_action_links($links, $file)
                {
                    ...
                    if ($file == $this_plugin) {
                        $settings_link = '<a href="'.admin_url('admin.php?page=pmpro-paymentsettings').'">'.__('Settings', KKD_SENANGPAYPMP).'</a>';
                ...
                static function pmpro_gateways($gateways)
                {
                    if (empty($gateways['senangpay'])) {
                        $gateways = array_slice($gateways, 0, 1) + array("senangpay" => __('SenangPay', KKD_SENANGPAYPMP)) + array_slice($gateways, 1);
                    }
                    return $gateways;
				}
                    ...
                    if(isset($_GET['status_id']) && isset($_GET['order_id']) && isset($_GET['msg']) && isset($_GET['transaction_id']) && isset($_GET['hash']))
                    {
                        ...
                        if($hashed_string == urldecode($_GET['hash']))
                        {
                            ...
                            if(urldecode($_GET['status_id']) == '1')
                            {
                                ...
                                $content = "<ul>
                                    <li><strong>".__('Account', KKD_SENANGPAYPMP).":</strong> ".$current_user->display_name." (".$current_user->user_email.")</li>
                                    <li><strong>".__('Order', KKD_SENANGPAYPMP).":</strong> ".$pmpro_invoice->code."</li>
                                    <li><strong>".__('Membership Level', KKD_SENANGPAYPMP).":</strong> ".$pmpro_level->name."</li>
                                    <li><strong>".__('Amount Paid', KKD_SENANGPAYPMP).":</strong> ".$pmpro_currency." ".$amount_total."</li>
                                </ul>";
                                ob_start();
